...
import {
  ...
  setRefetch,
  deleteItemsByIds,
} from '../../stores/collaborations/collaborationsSlice';
import { useAppDispatch, useAppSelector } from '../../stores/hooks';
import { useRouter } from 'next/router';
import { Field, Form, Formik } from 'formik';
import { DataGrid, GridColDef } from '@mui/x-data-grid';
import { loadColumns } from './configureCollaborationsCols';
...
const TableSampleCollaborations = ({
  filterItems,
  setFilterItems,
  filters,
  showGrid,
  ...
  const {
    ...
    loading,
    count,
    notify: collaborationsNotify,
    refetch,
  } = useAppSelector((state) => state.collaborations);
  ...
  useEffect(() => {
    if (collaborationsNotify.showNotification) {
      notify(
        collaborationsNotify.typeNotification,
        collaborationsNotify.textNotification,
      );
    }
  }, [collaborationsNotify.showNotification]);
  ...
  const handleViewAction = (id: string) => {
    router.push(`/collaborations/collaborations-view/?id=${id}`);
  };

  const handleCreateEventAction = ({ start, end }: SlotInfo) => {
    router.push(
      `/collaborations/collaborations-new?dateRangeStart=${start.toISOString()}&dateRangeEnd=${end.toISOString()}`,
    );
  };
    ...
    <>
      ...
      {!showGrid && (
        <BigCalendar
          ...
          pathEdit={`/collaborations/collaborations-edit/?id=`}
          pathView={`/collaborations/collaborations-view/?id=`}
...
export default TableSampleCollaborations;

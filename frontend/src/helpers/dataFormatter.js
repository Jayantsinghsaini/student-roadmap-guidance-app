...
export default {
  ...
  student_profilesManyListFormatter(val) {
    if (!val || !val.length) return [];
    return val.map((item) => item.user);
  },
  student_profilesOneListFormatter(val) {
    if (!val) return '';
    return val.user;
  },
  student_profilesManyListFormatterEdit(val) {
    if (!val || !val.length) return [];
    return val.map((item) => {
      return { id: item.id, label: item.user };
    });
  },
  student_profilesOneListFormatterEdit(val) {
    if (!val) return '';
